...
import { signInWithEmailAndPassword1 } from "../../config/firebase.config";
...
const Signin = () => {
  ...
  const handleSubmit = async () => {
    const res = await signInWithEmailAndPassword1(email, password);
    if (res === "success") {
      console.log(res);
      navigate("/");
    ...
    <>
      <section className="w-full h-full flex flex-col">
        ...
        <div className="flex w-full items-center flex-col gap-3 justify-center grow px-5 mt-[-100px]">
          <div>
            <h2 className="text-3xl font-bold">Sign in your account</h2>
            <p
